...
try {
    ...
    if (!empty($_POST["registerButton"])) {
        ...
        if (!validarSinNumeros($nombre) || !validarSinNumeros($apellido)) throw new Exception("El nombre o apellido no deben contener caracteres espeiales", 400);
        $telefono = $_POST["registerPhone"];
        if (strlen($telefono)!==9) throw new Exception("El telefono debe contener 9 numeros", 400);
        $email = $_POST["registerEmail"];
        if(!validarEmail($email)) throw new Exception("Email invalido", 400);
        $password = $_POST["registerPassword"];
        if (strlen($telefono)<8) throw new Exception("La contraseña debe contener minimo 8 carcteres", 400);
