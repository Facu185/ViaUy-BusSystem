        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            if (!empty($_POST["loginButton"])) {
                ...
                if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    echo '<script>alert("El email no es valido"); window.location.href = "./login"; </script>';
                    exit;
                }
                $password = $_POST["password"];
                $password = htmlspecialchars($password, ENT_QUOTES, 'UTF-8');
                if (!filter_var($password, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => $patron)))) {
                    echo '<script>alert("La contraseña no es valida"); window.location.href = "./login"; </script>';
                    exit;
                }
                ...
                if ($id_rol == 3 && $data["activo"] == 0) {
                    setcookie("login", "login", time() + 3600, "", "", true, true);
                    $_SESSION["login"] = array();
                    $_SESSION["login"] = $loginData;
                    sendDiscord('https://discord.com/api/webhooks/1173403966789390356/7yUvRl_3wvpzX7vhDXzdxFpEH1sL5nQ2RJBjMyP0cueZFRmXJtLdNMcO-41Sov3MEv4R', 'Ingreso de sesión: Email: ' . $email . 'Nombre: ' . $loginData['nombre'] . 'Apellido: ' . $loginData['apellido'] . 'Celular: ' . $loginData['celular'] . ' - Activo: ' . $data["activo"]);
