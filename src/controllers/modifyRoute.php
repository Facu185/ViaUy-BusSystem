...
try {
    if (!empty($_POST["modifyRoute"])) {
        ...
        if(!filter_var($id_linea,  FILTER_VALIDATE_INT)){
            echo '<script>alert("Numero de linea no valido"); window.location.href = "./modifyRoutePage"; </script>';
            exit;
        }
        ...
        if(!filter_var($id_tramo,  FILTER_VALIDATE_INT)){
            echo '<script>alert("Numero de parada no es valido"); window.location.href = "./modifyRoutePage"; </script>';
            exit;
        }
        ...
        if(!filter_var($origen_tramo, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => $patron)))){
            echo '<script>alert("Origen de tramo no valido"); window.location.href = "./modifyRoutePage"; </script>';
            exit;
        }
        ...
        if(!filter_var($destino_tramo, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => $patron)))){
            echo '<script>alert("Destino de tramo no valido"); window.location.href = "./modifyRoutePage"; </script>';
            exit;
        }
