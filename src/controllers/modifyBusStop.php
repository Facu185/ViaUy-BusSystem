...
try {
    if (!empty($_POST["modifyBusStop"])) {
        ...
        if(!filter_var($numero_parada,  FILTER_VALIDATE_INT)){
            echo '<script>alert("Numero de parada no valido"); window.location.href = "./modifyBusStop"; </script>';
            exit;
        }
        if(!filter_var($localizacion, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => $patron)))){
            echo '<script>alert("Localizacion no valida"); window.location.href = "./modifyBusStop"; </script>';
            exit;
        }
        if(!filter_var($latitud, FILTER_VALIDATE_FLOAT)){
            echo '<script>alert("La latitud solo puede contener numeros"); window.location.href = "./modifyBusStop"; </script>';
            exit;
        }
        if(!filter_var($longitud, FILTER_VALIDATE_FLOAT)){
            echo '<script>alert("La longitud solo puede contener numeros"); window.location.href = "./modifyBusStop"; </script>';
            exit;
        }
