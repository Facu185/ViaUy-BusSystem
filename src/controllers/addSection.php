    ...
    if (!empty($_POST["addSection"])) {
        ...
        for ($a = 0; $a <= $rep; $a++) {
            ...
            if(!filter_var($parada_origen, FILTER_VALIDATE_INT)){
                echo '<script>alert("La parada de origen solo puede contener numeros"); window.location.href = "./addBusSection"; </script>';
                exit;
            }
            if(!filter_var($rutas, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => $patron)))){
                echo '<script>alert("Las rutas solo pueden contener letras y numeros"); window.location.href = "./addBusSection"; </script>';
                exit;
            }
            if(!filter_var($parada_destino, FILTER_VALIDATE_INT)){
                echo '<script>alert("La parada de destino solo puede contener numeros"); window.location.href = "./addBusSection"; </script>';
                exit;
            }
            if(!filter_var($tipo_tramo, FILTER_VALIDATE_INT)){
                echo '<script>alert("El tipo de tramo solo puede contener numeros"); window.location.href = "./addBusSection"; </script>';
                exit;
            }
            if(!filter_var($distancia, FILTER_VALIDATE_INT)){
                echo '<script>alert("La distancia solo puede contener numeros"); window.location.href = "./addBusSection"; </script>';
                exit;
            }
            if(!filter_var($calles, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => $patron)))){
                echo '<script>alert("Las calles solo pueden contener letras y numeros"); window.location.href = "./addBusSection"; </script>';
                exit;
            }
            $contador2++;
