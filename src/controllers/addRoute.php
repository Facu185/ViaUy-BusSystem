...
try {
    if (!empty($_POST["addRoute"])) {
        ...
        if(!filter_var($origen_linea, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => $patron)))){
            echo '<script>alert("El origen de la linea solo puede contener letras y numeros"); window.location.href = "./addRoutes"; </script>';
        }
        if(!filter_var($destino_linea, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => $patron)))){
            echo '<script>alert("El destino de la linea solo puede contener letras y numeros"); window.location.href = "./addRoutes"; </script>';
        }
        ...
        if (!empty($horarios_repetios)) {
            echo '<script>alert("Esta unidad y esta linea ya tienen este horario asignado"); window.location.href = "./addRoutes"; </script>';
            exit;
        }
        ...
        for ($a = 1; $a <= $rep; $a++) {
            ...
            for ($i = $contador; $i <= $length; $i++) {
                ...
                if(!filter_var($id_tramo, FILTER_VALIDATE_INT)){
                    echo '<script>alert("La parada solo puede contener numeros"); window.location.href = "./addRoutes"; </script>';
                    exit;
                }
                if(!filter_var($origen_tramo, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => $patron)))){
                    echo '<script>alert("El origen del tramo solo puede contener letras y numeros"); window.location.href = "./addRoutes"; </script>';
                    exit;
                }
                if(!filter_var($destino_tramo, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => $patron)))){
                    echo '<script>alert("El destino del tramo solo puede contener letras y numeros"); window.location.href = "./addRoutes"; </script>';
                    exit;
                }
            }
        ...
        echo '<script>alert("Recorrido añadido con exito"); window.location.href = "./addRoutes"; </script>';
