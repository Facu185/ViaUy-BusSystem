...
try {
    if (!empty($_POST["addBusButton"])) {
        ...
        if (!filter_var($cantidad_asientos, FILTER_VALIDATE_INT)) {
            echo '<script>alert("La cantidad de asientos solo pueden contener numeros"); window.location.href = "./addBus"; </script>';
            exit;
        }
        ...
        if (!filter_var($tipo_asientos, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => $patron)))) {
            echo '<script>alert("El tipo de asiento solo puede contener letras y numeros"); window.location.href = "./addBus"; </script>';
            exit;
        }
        ...
        if (!filter_var($caracteristicas, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => $patron)))) {
            echo '<script>alert("Las caracteristicas solo pueden contener letras y numeros"); window.location.href = "./addBus"; </script>';
            exit;
        }
        ...
        echo '<script>alert("Unidad añadida con exito"); window.location.href = "./addBus"; </script>';
    }
