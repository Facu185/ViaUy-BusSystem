...
try {
    if (!empty($_POST["addBusStop"])) {
        ...
        if(!filter_var($numero_parada, FILTER_VALIDATE_INT)){
            echo '<script>alert("El numero de parada solo puede contener numeros"); window.location.href = "./addBusStopPage"; </script>';
            exit;
        }
        if(!filter_var($longitud, FILTER_VALIDATE_FLOAT)){
            echo '<script>alert("La longitud solo puede contener numeros"); window.location.href = "./addBusStopPage"; </script>';
            exit;
        }
        if(!filter_var($localizacion, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => $patron)))){
            echo '<script>alert("La localizacion solo puede contener letras y numeros"); window.location.href = "./addBusStopPage"; </script>';
            exit;
        }
        if(!filter_var($latitud, FILTER_VALIDATE_FLOAT)){
            echo '<script>alert("La latitud solo puede contener numeros"); window.location.href = "./addBusStopPage"; </script>';
            exit;
        }
